...
const RndNum = (a: number, b: number): number => {
...
/**
 * 生成 ZEGO API 签名
 * @param appId AppId（数字或字符串）
 * @param signatureNonce 随机字符串（16位十六进制）
 * @param serverSecret ServerSecret
 * @param timestamp 当前Unix时间戳（秒）
 * @returns 小写32位十六进制签名
 */
export function generateZegoSignature(appId: string | number, signatureNonce: string, serverSecret: string, timestamp: number): string {
