...
/**
 * 获取数字人形象列表
 * Action: GetDigitalHumanInfo
 * 请求参数: DigitalHumanId
 * 返回参数: DigitalHuman(Object)
 */
export async function POST(req: NextRequest) {
  try {
    const { DigitalHumanId } = await req.json();
    ...
    const queryString = generateQueryParamsString('GetDigitalHumanInfo', appId, serverSecret);
    const fullUrl = `https://${ZEGO_API_HOST}/?${queryString}`;
    console.log('完整请求URL:', fullUrl);

    // 业务参数放在 body
    const bodyParams = { DigitalHumanId };
