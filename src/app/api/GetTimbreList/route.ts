...
/**
 * 获取数字人形象列表
 * Action: GetTimbreList
 * 请求参数: Offset(Int, 可选), Limit(Int, 可选), DigitalHumanId
 * 返回参数: DigitalHumans(Array), Total(Int)
 */
export async function POST(req: NextRequest) {
  try {
    const { Offset = 0, Limit = 20, DigitalHumanId } = await req.json();
    ...
    const queryString = generateQueryParamsString('GetTimbreList', appId, serverSecret);
